 ...
 * 
 * Captures video frames from camera/screen and encodes them into JVID messages
...
public:
     ...
     */
    using FrameCallback = std::function<void(const JVIDMessage&)>;
...
public:
     ...
     */
    std::unique_ptr<JVIDMessage> encodeFrame(
...
private:
     ...
     */
    std::vector<JVIDMessage::JAMCamFeatures::FaceBBox> detectFaces(
        const std::vector<uint8_t>& frame_data,
        uint32_t width,
        uint32_t height
    );
    ...
    void applyAutoFraming(
        ...
        uint32_t& width,
        uint32_t& height,
        const std::vector<JVIDMessage::JAMCamFeatures::FaceBBox>& faces
    );
