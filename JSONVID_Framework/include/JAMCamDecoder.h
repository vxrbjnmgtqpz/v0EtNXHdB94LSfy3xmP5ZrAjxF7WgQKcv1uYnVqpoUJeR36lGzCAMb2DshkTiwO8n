 ...
 * 
 * Decodes JSONVID messages into video frames and renders them with
...
public:
     ...
     */
    using FrameDropCallback = std::function<void(const JSONVIDMessage&, const std::string& reason)>;
...
private:
    ...
    // Input frame queue
    LockFreeQueue<JSONVIDMessage> input_queue_;
...
public:
     ...
     */
    bool processMessage(const JSONVIDMessage& message);
     ...
     */
    std::unique_ptr<VideoBufferManager::FrameBuffer> decodeFrame(const JSONVIDMessage& message);
...
private:
     ...
     */
    bool decodeSingleFrame(const JSONVIDMessage& message, VideoBufferManager::FrameBuffer* output_buffer);
     ...
     */
    bool shouldDropFrame(const JSONVIDMessage& message) const;
