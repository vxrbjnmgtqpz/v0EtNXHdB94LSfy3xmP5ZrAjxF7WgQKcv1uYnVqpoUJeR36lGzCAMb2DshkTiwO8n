 ...
 * @brief Basic JAMCam demonstration of JSONVID ultra-low latency video streaming
 * 
...
struct DemoStats {
    ...
    void printSummary() const {
        std::cout << "\n=== JSONVID Demo Statistics ===" << std::endl;
...
private:
    ...
    std::function<void(const JSONVIDMessage&)> receiver_callback_;
...
public:
    ...
    void setReceiverCallback(std::function<void(const JSONVIDMessage&)> callback) {
        receiver_callback_ = callback;
    }
    
    bool transmit(const JSONVIDMessage& message) {
...
private:
    void initializeComponents() {
        ...
        auto resolution_dims = JSONVIDMessage::getResolutionDimensions(config_.resolution);
    ...
    void setupCallbacks() {
        ...
        encoder_->setFrameCallback([this](const JSONVIDMessage& message) {
        ...
        transport_->setReceiverCallback([this](const JSONVIDMessage& message) {
        ...
        decoder_->setFrameDropCallback([this](const JSONVIDMessage& message, const std::string& reason) {
            stats_.frames_dropped.fetch_add(1);
...
DemoConfig parseArguments(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--resolution" && i + 1 < argc) {
            ...
            std::cout << "JSONVID JAMCam Demo Usage:" << std::endl;
...
int main(int argc, char* argv[]) {
    std::cout << "JSONVID Framework - JAMCam Ultra-Low Latency Demo" << std::endl;
