...
public:
    ...
    void sendMIDIMessage(std::unique_ptr<JMID::MIDIMessage> message);
    using MIDIMessageHandler = std::function<void(std::unique_ptr<JMID::MIDIMessage>)>;
...
public:
    ...
    void routeMIDIMessage(const std::string& sessionId, 
                         std::unique_ptr<JMID::MIDIMessage> message,
