...
std::vector<std::complex<double>> simple_fft(const std::vector<float>& signal) {
    ...
    for (size_t k = 0; k < N; ++k) {
        ...
        for (size_t n = 0; n < N; ++n) {
            ...
            sum += static_cast<double>(signal[n]) * std::complex<double>(cos(angle), sin(angle));
        }
        result[k] = sum;
    }
    return result;
...
AudioQualityAnalyzer::QualityMetrics AudioQualityAnalyzer::analyzeQuality(
    ...
    // Stereo imaging (for multi-channel) - simplified placeholder
    // Note: These fields may not exist in the actual struct
...
AudioQualityAnalyzer::PhaseMetrics AudioQualityAnalyzer::analyzePhaseLinearity(
    ...
    metrics.linearity_score = 0.95; // Simplified placeholder
    metrics.group_delay_variation_ms = 0.1;
    return metrics;
}
