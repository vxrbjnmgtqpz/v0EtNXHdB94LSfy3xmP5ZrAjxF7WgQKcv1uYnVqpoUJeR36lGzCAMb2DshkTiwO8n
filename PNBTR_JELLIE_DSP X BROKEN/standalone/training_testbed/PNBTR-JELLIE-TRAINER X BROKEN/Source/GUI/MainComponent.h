...
private:
    ...
    std::chrono::steady_clock::time_point lastTimerCall;
    ...
    // 🎮 GAME ENGINE SYSTEM: Your sophisticated AudioEngine architecture
    std::unique_ptr<class AudioEngine> audioEngine;

    // Audio device manager and device selectors (legacy - replaced by AudioEngine)
