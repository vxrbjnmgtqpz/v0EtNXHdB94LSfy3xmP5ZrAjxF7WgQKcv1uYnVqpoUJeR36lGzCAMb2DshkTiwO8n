...
PNBTRTrainer::PNBTRTrainer()
{
    ...
    // 🔥 CRITICAL: Initialize real audio input buffers for GPU engine
    realAudioInputBufferA.resize(2048 * 2, 0.0f); // Double stereo buffer size
    realAudioInputBufferB.resize(2048 * 2, 0.0f);
    ...
    // 🔧 ASYNC Metal initialization to prevent UI thread blocking
    juce::Logger::writeToLog("[PNBTRTrainer] Starting Metal initialization (ASYNC BACKGROUND)...");
    
    // Launch Metal initialization on background thread
    std::thread([this]() {
        ...
        metalInitialized.store(true);
    }).detach();
    
    juce::Logger::writeToLog("[PNBTRTrainer] Metal bridge initializing in background - constructor completing immediately");
}
...
void PNBTRTrainer::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    bool isMetalReady = metalInitialized.load() && MetalBridge::getInstance().isInitialized();
    ...
    if (++debugCounter % 100 == 0) { // Every ~2 seconds at 48kHz
        juce::Logger::writeToLog("[GPU BLOCK CHECK] Training Active: " + juce::String(isTrainingActive ? "YES" : "NO") + 
                                ...
                                ", Metal Async Init: " + juce::String(metalInitialized.load() ? "COMPLETE" : "IN PROGRESS"));
    }
...
//==============================================================================
// 🔥 CRITICAL: Real audio input bridge for GPU engine pipeline

void PNBTRTrainer::setLatestRealAudioInput(const juce::AudioBuffer<float>& input)
{
    // Thread-safe buffer swap - called by MainComponent audio callback
    bool toggle = !realAudioToggle.load(std::memory_order_relaxed);
    std::vector<float>& targetBuffer = toggle ? realAudioInputBufferA : realAudioInputBufferB;
    
    const int numSamples = input.getNumSamples();
    const int numChannels = input.getNumChannels();
    
    // Ensure we have enough space
    if (targetBuffer.size() < numSamples * 2) {
        targetBuffer.resize(numSamples * 2, 0.0f);
    }
    
    // Copy audio data with stereo interleaving
    const float* leftChannel = input.getReadPointer(0);
    const float* rightChannel = numChannels > 1 ? input.getReadPointer(1) : leftChannel;
    
    for (int i = 0; i < numSamples; ++i) {
        targetBuffer[i * 2] = leftChannel[i];      // Left
        targetBuffer[i * 2 + 1] = rightChannel[i]; // Right
    }
    
    // Atomic swap - now AudioScheduler can access new data
    realAudioToggle.store(toggle, std::memory_order_release);
    recentAudioInputAvailable.store(true, std::memory_order_release);
    
    static int debugCount = 0;
    if (++debugCount % 100 == 0) { // Every ~2 seconds
        juce::Logger::writeToLog("[REAL AUDIO BRIDGE] Transferred " + juce::String(numSamples) + 
                                " samples to GPU engine pipeline");
    }
}

void PNBTRTrainer::getLatestInputForScheduler(juce::AudioBuffer<float>& buffer)
{
    // Thread-safe buffer read - called by AudioScheduler
    bool toggle = realAudioToggle.load(std::memory_order_acquire);
    const std::vector<float>& sourceBuffer = toggle ? realAudioInputBufferA : realAudioInputBufferB;
    
    const int requestedSamples = buffer.getNumSamples();
    const int availableSamples = static_cast<int>(sourceBuffer.size()) / 2; // Stereo interleaved
    const int actualSamples = std::min(requestedSamples, availableSamples);
    
    // Ensure output buffer has correct size
    buffer.setSize(2, actualSamples, false, true, true);
    
    // De-interleave stereo data back to JUCE format
    float* leftChannel = buffer.getWritePointer(0);
    float* rightChannel = buffer.getWritePointer(1);
    
    for (int i = 0; i < actualSamples; ++i) {
        leftChannel[i] = sourceBuffer[i * 2];      // Left
        rightChannel[i] = sourceBuffer[i * 2 + 1]; // Right
    }
    
    static int debugCount = 0;
    if (++debugCount % 100 == 0) { // Every ~2 seconds
        juce::Logger::writeToLog("[GPU ENGINE INPUT] Retrieved " + juce::String(actualSamples) + 
                                " samples for Metal pipeline processing");
    }
}
