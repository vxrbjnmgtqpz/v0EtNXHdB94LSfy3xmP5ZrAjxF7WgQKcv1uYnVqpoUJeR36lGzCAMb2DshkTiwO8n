...
public:
    ...
    // 🔥 CRITICAL: Real audio input connection for GPU engine
    bool hasRecentAudioInput() const { return recentAudioInputAvailable.load(); }
    void getLatestInputForScheduler(juce::AudioBuffer<float>& buffer);
    void setLatestRealAudioInput(const juce::AudioBuffer<float>& input); // Called by MainComponent
...
private:
    ...
    std::atomic<bool> metalInitialized{false}; // Track async Metal initialization
    std::atomic<bool> recentAudioInputAvailable{false}; // Track real audio input availability
    
    // 🎮 Real audio input bridge for GPU engine (double buffered)
    std::vector<float> realAudioInputBufferA;
    std::vector<float> realAudioInputBufferB;
    std::atomic<bool> realAudioToggle{false};
