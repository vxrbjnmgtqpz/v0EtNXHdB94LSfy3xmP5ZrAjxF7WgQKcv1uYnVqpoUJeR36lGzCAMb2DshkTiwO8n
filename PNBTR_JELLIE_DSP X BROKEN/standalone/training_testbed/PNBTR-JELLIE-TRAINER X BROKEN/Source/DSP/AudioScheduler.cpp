...
void AudioScheduler::processAudioBlock()
{
    ...
    // 🔥 CRITICAL FIX: Connect real audio input to GPU engine pipeline
    // Get the latest real audio input from the shared buffer system
    juce::AudioBuffer<float> audioBuffer(2, blockSize); // Stereo buffer
    juce::MidiBuffer midiBuffer;
    
    // 🎮 GPU ENGINE: Get real microphone input from trainer's input buffers
    if (trainer->hasRecentAudioInput()) {
        // Use actual microphone input captured by JUCE AudioDeviceManager
        trainer->getLatestInputForScheduler(audioBuffer);
        
        std::cout << "[AudioScheduler] 🎮 GPU ENGINE: Processing REAL AUDIO INPUT (" << blockSize << " samples)" << std::endl;
    } else {
        // Fallback to silence only if no real input available
        audioBuffer.clear();
        std::cout << "[AudioScheduler] ⚠️  No real audio input - using silence fallback" << std::endl;
    }
    
    // 🎮 REAL GPU PIPELINE: Process actual microphone input through your sophisticated engine
    try {
        trainer->processBlock(audioBuffer, midiBuffer);
        std::cout << "[AudioScheduler] 🔥 GPU PIPELINE ACTIVE: Processed real audio through Metal kernels" << std::endl;
    } catch (const std::exception& e) {
        std::cout << "[AudioScheduler] Exception in GPU pipeline: " << e.what() << std::endl;
    }
}
