...
JMIDIntegrationPanel::JMIDIntegrationPanel()
    : titleLabel("titleLabel", "JMID Framework Integration")
    ...
    , benchmarkButton("Run Benchmark")
{
    ...
    // bassoonParser = std::make_unique<JMID::BassoonParser>();
    // profiler = std::make_unique<JMID::PerformanceProfiler>();
...
void JMIDIntegrationPanel::paint(juce::Graphics& g)
{
...
void JMIDIntegrationPanel::resized()
{
...
void JMIDIntegrationPanel::timerCallback()
{
    updatePerformanceDisplay();
    processMessageQueue();
}

void JMIDIntegrationPanel::sendMessageClicked()
{
    try {
        logMessage("Creating and sending JMID message...");
...
void JMIDIntegrationPanel::sendBurstClicked()
{
    auto startTime = std::chrono::high_resolution_clock::now();
    int successCount = 0;
...
void JMIDIntegrationPanel::validateSchemaClicked()
{
    try {
...
void JMIDIntegrationPanel::benchmarkClicked()
{
...
void JMIDIntegrationPanel::updatePerformanceDisplay()
{
...
void JMIDIntegrationPanel::processMessageQueue()
{
    std::shared_ptr<JMID::MIDIMessage> message;
...
void JMIDIntegrationPanel::logMessage(const juce::String& message)
{
...
std::shared_ptr<JMID::MIDIMessage> JMIDIntegrationPanel::createSelectedMessage()
{
    ...
    switch (messageType) {
        case 1: // Note On
            return std::make_shared<JMID::NoteOnMessage>(channel, note, velocity, timestamp);
        case 2: // Note Off
            return std::make_shared<JMID::NoteOffMessage>(channel, note, velocity, timestamp);
        case 3: // Control Change
            return std::make_shared<JMID::ControlChangeMessage>(channel, note, velocity, timestamp); // Using note as controller number
        case 4: // System Exclusive
        {
            ...
            return std::make_shared<JMID::SystemExclusiveMessage>(manufacturerId, sysexData, timestamp);
        }
        default:
            return nullptr;
    }
