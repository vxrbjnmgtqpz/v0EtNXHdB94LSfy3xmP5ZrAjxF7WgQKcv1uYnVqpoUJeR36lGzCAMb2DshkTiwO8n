...
layout(std430, binding = 0) readonly buffer RecentSamples {
    float recentSamples[WINDOW_SIZE];
};

layout(std430, binding = 1) writeonly buffer PredictedSamples {
    float predictedSamples[PREDICT_FRAMES * WINDOW_SIZE];
};
...
void main() {
    ...
    for (uint i = 0; i < WINDOW_SIZE; ++i) {
        float hann = 0.5 * (1.0 - cos(2.0 * 3.14159265 * float(i) / float(WINDOW_SIZE - 1)));
    ...
    for (uint k = 0; k < FFT_SIZE; ++k) {
        ...
        for (uint n = 0; n < WINDOW_SIZE; ++n) {
            float angle = 2.0 * 3.14159265 * float(k * n) / float(WINDOW_SIZE);
    ...
    for (uint f = 0; f < PREDICT_FRAMES; ++f) {
        for (uint n = 0; n < WINDOW_SIZE; ++n) {
            float sample = 0.0;
            for (uint k = 0; k < FFT_SIZE; ++k) {
                float shiftedPhase = phase[k] + float(f) * 2.0 * 3.14159265 * float(k) / float(WINDOW_SIZE);
                sample += mag[k] * cos((2.0 * 3.14159265 * float(k) * float(n) / float(WINDOW_SIZE)) + shiftedPhase);
            }
